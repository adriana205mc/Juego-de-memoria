  ...
  if (currentMove < 2 && !gameFinished) {
    if (
      ...
      if (++currentMove === 2) {
        ...
        if (
          selectedCards[0].querySelector(".face").style.backgroundImage ===
          selectedCards[1].querySelector(".face").style.backgroundImage
        ) {
          selectedCards = [];
          currentMove = 0;
...
function setupCards() {
  ...
  for (let i = 0; i < totalCards; i++) {
    ...
    const faceDiv = div.querySelector(".face");
    faceDiv.style.backgroundImage = getFaceValue(valuesUsed[i]);
